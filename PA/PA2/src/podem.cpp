...
int ATPG::podem(const fptr fault, int &current_backtracks)
{
         ...
         !(find_test && (attempt_num == total_attempt_num)))
  {
    ...
    // Generate N-detect patterns for a single fault.
    bool Ndet_flag;
    do
    {
      Ndet_flag = false;
...
ATPG::wptr ATPG::find_pi_assignment(const wptr object_wire, const int &object_level)
{
  ...
  {
    /* TODO */
    // implement the backtrace algorithm according to PODEM paper.
    // Fig 9.3
    switch (object_wire->inode.front()->type)
    {
    // Fig 9.5: NAND/OR with Objective = 1 -> easiest_control / Fig 9.4: NAND/OR with Objective = 0 -> hardest_control
    case OR:
      new_object_wire = (object_level) ? find_easiest_control(object_wire->inode.front()) : find_hardest_control(object_wire->inode.front());
      break;
    case NAND:
      new_object_wire = (object_level) ? find_easiest_control(object_wire->inode.front()) : find_hardest_control(object_wire->inode.front());
      break;
    // Fig 9.4: AND/NOR with Objective = 1 -> hardest_control / Fig 9.5: AND/NOR with Objective = 0 -> easiest_control
    case NOR:
      new_object_wire = (object_level) ? find_hardest_control(object_wire->inode.front()) : find_easiest_control(object_wire->inode.front());
      break;
    case AND:
      new_object_wire = (object_level) ? find_hardest_control(object_wire->inode.front()) : find_easiest_control(object_wire->inode.front());
      break;
    ...
    {
    case NOT:
    // Fig 9.8: NAND/NOR -> next objective level should be complement
    case NOR:
    ...
    case NAND:
      new_object_level = object_level ^ 1;
      break;
    // Fig 9.8: Otherwise -> next objective level should be the same
    case BUF:
    case AND:
...
bool ATPG::trace_unknown_path(const wptr w)
{
  int i, nout;
  /* TODO search X-path*/
  // HINT if w is PO, return TRUE
  if (w->is_output() && w->value == U)
    return true;

  // if not, check all its fanout.
  nout = static_cast<int>(w->onode.size());
  for (i = nout - 1; i >= 0; --i)
  {
    if (w->onode[i]->owire.front()->value == U && trace_unknown_path(w->onode[i]->owire.front()))
      return true;
  }
...
int ATPG::set_uniquely_implied_value(const fptr fault)
{
  ...
  // HINT: use backward_imply() function to check if fault can be excited or not.
  //  Set output of GUT opposite to its stuck fault value (Fig.8.3),  and do backward implication.

  if (fault->fault_type) // SA1
  {
    switch (backward_imply(w, 0))
    {
    case TRUE:
      pi_is_reach = TRUE;
      break;
  ...
  else if (fault->fault_type == 0) // SA0
  {
    switch (backward_imply(w, 1))
    {
    case TRUE:
      pi_is_reach = TRUE;
      break;
 ...
 *return CONFLICT if conlict with previous assignment */
