...
void ATPG::sim()
{
  ...
  for (wptr input : cktin)
  {
    ...
    {
      gate->set_marked(); // mark the gate
    }
    input->remove_changed(); // remove the changed flag of input
  }
  /*
  ...
  for (int i = ncktin; i < nckt; i++)
  {
    ...
    if (gate->is_marked()) // check if the gate is marked
    {
      evaluate(gate);
      gate->remove_marked(); // remove the mark of the gate
    }
    if (wire->is_changed()) // check if the wire is changed
    {
      for (nptr gate_c : wire->onode)
        gate_c->set_marked(); // mark the gates
      wire->remove_changed(); // remove the changed flag of the wire
    }
  }
  /*end of TODO*/
} /* end of sim */
