...
int ATPG::podem(const fptr fault, int &current_backtracks) {
  ...
  while ((no_of_backtracks < backtrack_limit) && !no_test &&
    ...
    /* this generate_pattern_again while loop is to generate multiple patterns for a single fault 
     * this part is NOT in the original PODEM paper  */
    bool generate_pattern_again;
    do{
        generate_pattern_again = false;
        if (wpi) {
            sim();
...
int ATPG::set_uniquely_implied_value(const fptr fault) {
  ...
  // Set output of GUT opposite to its stuck fault value (Fig.8.3), 
  // and then do backward implication. (this is not shown in Fig. 8.3)
  /*
  switch(  )
  {
      case TRUE:
