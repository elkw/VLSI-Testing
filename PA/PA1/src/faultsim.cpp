    ...
    if ((num_of_fault == num_of_faults_in_parallel) || (next(pos, 1) == flist_undetect.cend()))
    {
      ...
      while (!wlist_faulty.empty())
      {
        ...
        if (w->is_output())
        {
          ...
          {
            if (!((w->wire_value_g ^ Unknown[i]) & Mask[i]))
              continue;
            if (!((w->wire_value_g ^ w->wire_value_f) & Mask[i]))
              continue;
...
void ATPG::fault_sim_evaluate(const wptr w)
{
  ...
  if (w->wire_value_g != new_value)
  {
     ...
     * Then set this wire to be faulty by set_faulty() and nsert it into the faulty_wire list if necessary.
