...
ATPG::wptr ATPG::wfind(const string &name)
{
  ...
  {
    if (name == pos->name)
      return pos.get(); // "return *pos" would return a unique_ptr obj, that's wrong.
  }
  return (nullptr);
} /* end of wfind */
...
void ATPG::create_structure()
{
  ...
  for (i = 0; i < HASHSIZE; i++)
  {
    ...
    { // for each node n in the circuit
      n = pos.get();

      for (wptr w : n->iwire)
      { // insert node n into the onode of its iwires
        w->onode.push_back(n);
      }

      for (wptr w : n->owire)
      { // insert node n into the inode of its owires
        w->inode.push_back(n);
      } // for w
    }   // for n
  }     // for i
...
void ATPG::read_vectors(const string &vetFile)
{
  ...
  while (!file.eof() && !file.bad())
  {
    ...
    if (t[0] != 'T')
      continue; // if this line is not a vector, ignore it
    else
    {
      vec.clear();
      for (char c : t)
