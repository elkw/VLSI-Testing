...
void ATPG::level_circuit()
{
  ...
  */
  ...
  {                                // go through all nodes in the event list (for this level)
    ncurrent = event_list.front(); // get a node from the front of event list
    event_list.pop_front();
    schedule = true;
    for (wptr wptr_ele : ncurrent->iwire)
    { // check every gate input
      if (!(wptr_ele->is_marked()))
      {                   // if any one gate input is not marked yet
    ...
    if (schedule)
    {
      for (wptr wptr_ele : ncurrent->owire)
      {
        ...
        for (nptr nptr_ele : wptr_ele->onode)
        {                                                                               // propagate the event to fanout node
    ...
    if (ncurrent == nprelast)
    {
      ...
      { // pop every wire in the stack and mark them
...
void ATPG::rearrange_gate_inputs()
{
  ...
  {
    if (n = sort_wlist[i]->inode.front())
    { // check every gate in the circuit
      for (int j = 0; j < n->iwire.size(); j++)
      { // check every pait of gate inputs
